...
#  2) 幂等检查看 LED class 设备(power/disk1/netdev)而非 i2c 客户端节点(6-003a)；
#  3) Restart=on-failure + TimeoutStartSec=120 兜底极慢的磁盘检查/枚举；
#  4) 全部逻辑放进 /usr/local/sbin/ugreen-led-init.sh 脚本文件，由 ExecStart 直接调用，
#     避免在 unit 里写 ExecStart=/bin/sh -c '...'。systemd 对 -c 字符串中
#     "命令替换 $(...) + 循环" 的解析有怪异行为：循环里赋值的 BUS 在末尾 echo 处会被
#     吞成空串，导致 "i2c-/new_device" 报错、冷重启后 LED 不亮。独立脚本文件可彻底规避
#     (已实测 journal 显示 BUS=[6] 正常)。
TimeoutStartSec=120
Restart=on-failure
RestartSec=5
ExecStart=/usr/local/sbin/ugreen-led-init.sh

[Install]
WantedBy=multi-user.target
'''

UGREEN_LED_INIT_SCRIPT = r'''#!/bin/sh
...
def _deploy_robust_files(result):
    ...
    files = [
        ...
        ("/usr/local/sbin/ugreen-led-init.sh", UGREEN_LED_INIT_SCRIPT, 0o755),
